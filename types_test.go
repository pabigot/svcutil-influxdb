...
func TestWriteFailedPermanently(t *testing.T) {
	sub := fmt.Errorf("test error")
	err := &errWriteFailedPermanently{sub}
	confirmError(t, err, ErrWriteFailedPermanently, "write failed permanently: test error")
...
func TestStringSet(t *testing.T) {
	ss := makeStringSet(nil)
	if ss != nil {
		t.Fatal("nil set is not nil")
	}
	if v := ss.elements(); v != nil {
		t.Errorf("nil set has non-nil elements: %v", v)
	}
	if ss.has("a") {
	...
	if ss.has("a") {
		t.Fatal("empty set found a")
	}

	ss = makeStringSet([]string{"a", "b"})
	if l := len(ss); l != 2 {
		t.Errorf("set length: %d", l)
	}
	if !ss.has("a") || !ss.has("b") {
		t.Fatal("set not populated")
	}
	ss = ss.add("c")
	if v := len(ss); v != 3 || !ss.has("c") {
		t.Errorf("add didn't work: %d, %t", v, ss.has("c"))
	}

	ss = nil
	ss = ss.add("d")
	if v := len(ss); v != 1 || !ss.has("d") {
		t.Errorf("add to nil didn't work: %d, %t", v, ss.has("d"))
	}

	ab := makeStringSet([]string{"a", "b"})
	if v := ab.elements(); !reflect.DeepEqual(v, []string{"a", "b"}) {
		t.Errorf("wrong elements: %v", v)
	}
	bc := makeStringSet([]string{"c", "b"})
	if v := bc.elements(); !reflect.DeepEqual(v, []string{"b", "c"}) {
		t.Errorf("wrong elements: %v", v)
	}
	ss = ab.minus(nil)
	if !reflect.DeepEqual(ss, ab) {
		t.Error("minus nil wrong")
	}
	ss = ab.minus(bc)
	if v := len(ss); v != 1 || !ss.has("a") {
		t.Errorf("non-empty minus wrong")
	}
	ss = ab.minus(ab)
	if ss != nil {
		t.Errorf("empty minus wrong")
	}
}
...
func TestConnectionImplStructure(t *testing.T) {
	...
	confirmError(t, err, config.ErrConnection, ": nil")
...
func TestConnectionStateImpl(t *testing.T) {
	...
	if !cs.Permanent {
		t.Error("netError expected permanent")
	}
...
func TestConnectionStateRetryPolicy(t *testing.T) {
	...
	confirmError(t, cs.Err, ErrConnNotReady, "not ready")
	if !cs.Permanent {
		t.Error("BackoffStop not marked permanent")
	}
...
func TestConnectionShutdownDuringCheckReady(t *testing.T) {
	...
	rdv := make(chan context.Context, 1)
	mc := mockClient{
		ready: func(ctx context.Context) (*domain.Ready, error) {
	...
	<-inRdy
	conn.SetLogPriority(lw.Debug)
	conn.Shutdown(true)

	cap := <-rdv
	confirmError(t, cap.Err(), context.Canceled, "context canceled")

	// The resident command handler completes SetLogPriority but doesn't
	...
	if !cs.Permanent {
		t.Errorf("shutdown cs not permanent")
	}
...
func TestBucketSetWriteHandler(t *testing.T) {
	...
	wsc := func(batch Batch, err error, bs BucketState) WriteRecovery {
		return WriteRecoveryDrop
	}
	b.SetWriteSettledCallback(wsc)
...
func TestBucketHandleWriteSettled(t *testing.T) {
	...
	// NB: Does not include the 3/93 from the wsc call
	if v := bs.HeldSize; !v.Equal(Cardinality{8, 252}) {
		t.Errorf("bad held state: %v", v)
	}
	if v := bs.MetricsWritten; v != 0 { // not written at wsc
		t.Errorf("bad write count: %v", v)
	}
	if v := bs.MetricsDropped; v != 3 {
		t.Errorf("bad drop count: %v", v)
	...
	// Fail again, this time requeue
	wab.retErr = merr
	ws.wrr = WriteRecoveryRequeue
	goCh <- chGo // Release the third transmission
	<-wscch      // Acknowledge the write settled callback
	calls = ws.calls()
	wscch = ws.requestNotify()

	// Verify settled from third transmission
	if v := len(calls); v != 2 {
		t.Errorf("wsc called: %v", v)
	}
	call = calls[1]
	...
	<-readyCh // Wait for the fourth transmission
	bs = b.State()
	t.Logf("bs3 %v", bs)
	if v := bs.ReadyBatchCount; v != 1 {
		t.Errorf("bad ready batch count: %v", v)
	...
	if v := bs.PendingSize; !v.Equal(Cardinality{2, 66}) {
		t.Errorf("bad pending state: %v", v)
	}
	if v := bs.HeldSize; !v.Equal(Cardinality{5, 159}) {
		t.Errorf("bad held state: %v", v)
	...
	wab.retErr = &errWriteFailedPermanently{merr}
	goCh <- chGo // Release the fourth transmission
	<-wscch      // Acknowledge the write settled callback
	calls = ws.calls()
	wscch = ws.requestNotify()

	// Verify settled from fourth transmission
	if v := len(calls); v != 3 {
		t.Errorf("wsc called: %v", v)
	}
	call = calls[1]
	...
	<-readyCh // Wait for the fifth transmission
	bs = b.State()
	t.Logf("bs4 %v", bs)
	// confirms requeued
	if v := bs.ReadyBatchCount; v != 0 {
		t.Errorf("bad ready batch count: %v", v)
	}
	if v := bs.ReadySize; !v.Equal(Cardinality{0, 0}) {
		t.Errorf("bad ready state: %v", v)
	}
	if v := bs.PendingSize; !v.Equal(Cardinality{3, 93}) {
		t.Errorf("bad pending state: %v", v)
	}
	if v := bs.HeldSize; !v.Equal(Cardinality{3, 93}) {
		t.Errorf("bad held state: %v", v)
	}
	if v := bs.MetricsWritten; v != 3 {
		t.Errorf("bad write count: %v", v)
	}
	if v := bs.MetricsDropped; v != 5 { // wrr was success, but permanent error overrode
		t.Errorf("bad drop count: %v", v)
	}

	goCh <- chGo // Release the fifth transmission
	<-wscch      // Acknowledge the write settled callback
...
func TestBucketCheckMetric(t *testing.T) {
	...
	b := &Bucket{
		sch: map[string]*Schema{
			"mn": {},
			"mr": {
				reqTags: makeStringSet([]string{"t1"}),
			},
			"mf": {
...
func (ws *writeSettler) defaultCallback(batch Batch, err error, bs BucketState) WriteRecovery {
...
` {
	...
	confirmError(t, wv.err, ErrWriteFailedPermanently, "bucket terminated")
