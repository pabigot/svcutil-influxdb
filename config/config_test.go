...
func TestValidateConnection(t *testing.T) {
	...
	cp = &Connection{
		...
		Precision: Duration(5 * time.Second),
		Buckets: map[string]*Bucket{
			...
			"b2": {
				...
				Options: &BucketOptions{
					FlushInterval: Duration(time.Nanosecond), // invalid
				},
			},
		},
	}
	...
	cp.Precision = Duration(time.Nanosecond)

	cp.ReadyCheckTimeout = Duration(-1 * time.Second)
	confirmError(t, cp.Validate(), ErrConfig,
		"ReadyCheckTimeout (-1s) must be at least 500ms")
	cp.ReadyCheckTimeout = Duration(time.Nanosecond)
	confirmError(t, cp.Validate(), ErrConfig,
		"ReadyCheckTimeout (1ns) must be at least 500ms")
	cp.ReadyCheckTimeout = 0

	cp.ReadyCheckInterval = Duration(-1 * time.Second)
	confirmError(t, cp.Validate(), ErrConfig,
		"ReadyCheckInterval (-1s) must be at least 2s")
	cp.ReadyCheckInterval = Duration(time.Duration(cp.ReadyCheckTimeout) - 500*time.Millisecond)
	confirmError(t, cp.Validate(), ErrConfig,
	...
	b2.Options.FlushInterval = Duration(0)
...
func TestValidateMeasurementName(t *testing.T) {
	...
	b := cp.Buckets["b"]
	b.Measurements = make(map[string]*Schema)
...
func TestBucketOptionsValidate(t *testing.T) {
	...
	b.FlushInterval = Duration(5 * time.Millisecond)
	...
	b.PendingTimeout = Duration(time.Second)
